...
def convert(input_nifti_path: str, input_dicom_path: str, output_dicom_path: str, ROI_name: str, ROI_color: str):
    ...
    for z in range(volume.shape[2]):
        ...
        allContourCoordinates.append(contourCoordinatesThisSlice)
    ...
    if True:
        ...
        for slice in allContourCoordinates:
            for contour in slice:
                ax.plot(contour[0::3], contour[1::3], contour[2::3],'b.-')
    ...
    rt_refd_study1.ReferencedSOPInstanceUID = ds.StudyInstanceUID # '1.3.6.1.4.1.9590.100.1.2.201285932711485367426568006803977990318'
    ...
    for ROI in range(1,numberOfROIs+1):
        ...
        for z in range(volume.shape[2]):
            ...
            numberOfContoursInThisSlice = len(allContourCoordinates[z])
            for c in range(numberOfContoursInThisSlice):

                currentCoordinates = allContourCoordinates[z][c]
    ...
    ds.StudyDescription = "studDesc"
    ds.ManufacturerModelName = "manufModName"
    ds.PatientName = "19849"
    ds.DeviceSerialNumber = "devSerNum"
    ds.SoftwareVersions = "softVer"
    ds.StudyID = "studId"
    ds.SeriesNumber = "1235"
    del(ds.ImageType)
    del(ds.ContentDate)
    del(ds.AcquisitionDateTime)
    del(ds.ContentTime)
    del(ds.MRAcquisitionType)
    del(ds.SliceThickness)
    del(ds.RepetitionTime)
    del(ds.EchoTime)
    del(ds.MagneticFieldStrength)
    del(ds.EchoTrainLength)
    del(ds.PatientPosition)
    del(ds.ImagePositionPatient)
    del(ds.ImageOrientationPatient)
    del(ds.FrameOfReferenceUID)
    del(ds.PositionReferenceIndicator)
